...
# Ensure the upload subdirectory exists and is writable
MEDIA_DIR="$LABEL_STUDIO_BASE_DATA_DIR/media"
UPLOAD_DIR="$MEDIA_DIR/upload"

for dir in "$MEDIA_DIR" "$UPLOAD_DIR"; do
    if [ ! -d "$dir" ]; then
        echo >&3 "=> Creating directory: $dir"
        mkdir -p "$dir" || {
            echo >&3 "ERROR: Failed to create directory: $dir"
            echo >&3 "  Ensure the data volume is writable by UID $(id -u)"
            exit 1
        }
    fi
done
